...
routeData['numTrips'] <- lapply(routeData['numTrips'], as.character)
routeData['numTrips'] <- lapply(routeData['numTrips'], as.numeric)
...
avg1 <- median(subset(routeData, Routes == 1, select = c('numTrips'))$numTrips)
avg2 <- median(subset(routeData, Routes == 2, select = c('numTrips'))$numTrips)
avg3 <- median(subset(routeData, Routes == 3, select = c('numTrips'))$numTrips)
avg4 <- median(subset(routeData, Routes == 4, select = c('numTrips'))$numTrips)
...
p <- ggplot(routeData, aes(x = `numTrips`, y = `Routes`)) +
  geom_density_ridges_gradient(aes(fill = ..x..), scale = 3, size = 0.3 ) +
  scale_fill_gradientn(colours=Colormap, name = "# trips") + 
  coord_cartesian(xlim = c(0, 150))
p <- p + theme_linedraw() + 
  theme(panel.grid.minor = element_line(colour = "#333333", size = 1.0)) + 
  scale_x_continuous(name="# trips", minor_breaks = seq(0 , 150, 30), breaks = seq(0, 150, 30)) + 
  scale_y_discrete(name="# routes")
# p <- p + ggplot(average, aes(x=average$'avg', y=average$'y', group = average$'grp')) + 
#   geom_point() + geom_line()
p
ggsave(paste(dataPath, "routes_numTrips_joyplot.pdf", sep=''),
       width = 5, height = 3)
