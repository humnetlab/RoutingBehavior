...
class Clustering:
    ...
    def modHausDist(self, t1idx, t2idx):
        """Computes modified Hausdorff distance."""
...
#         return distances[int(round((len(distances) - 1) * self.alpha))]
    ...
    def createDistanceMatrix(self, userId):
...
#                 self.distMat[c, r] *= dist
        # pickle.dump(self.distMat, open("distMat_" + str(zonePair) + ".pkl", 'wb'), pickle.HIGHEST_PROTOCOL)
        # pickle.dump(self.distMat, open(dataPath + "userData/distMatrix/distMat_" + str(userId) + ".pkl", 'wb'), pickle.HIGHEST_PROTOCOL)
    ...
    def clusterAgglomerartive(self, trajectories, cn, zonePair=3):
        """
        ...
        if not os.path.isfile("distMat_" + str(zonePair) + ".pkl"):
            self.createDistanceMatrix(zonePair)
        else:
            self.distMat = pickle.load(open("distMat_" + str(zonePair) + ".pkl", 'rb'))
    ...
    def clusterSpectral(self, trajectories, clusters=-1, userId=3):
        """
        input:
        ...
        if userId == 0:
            self.createDistanceMatrix(userId)
            # self.distMat = pickle.load(open(dataPath + "userData/distMatrix/distMat_" + str(userId) + ".pkl", 'rb'))
            return self.distMat

        # Update a distance matrix and std deviations
        if not os.path.isfile(dataPath + "userData/distMatrix/distMat_" + str(userId) + ".pkl"):
            self.createDistanceMatrix(userId)
        else:
            self.distMat = pickle.load(open(dataPath + "userData/distMatrix/distMat_" + str(userId) + ".pkl", 'rb'))

        print(np.min(self.distMat), np.max(self.distMat))

        fig = plt.figure()
        ax = plt.gca()
        im = plt.imshow(self.distMat)
        ...
        plt.tight_layout()
        plt.savefig(dataPath + "userData/distMatrix/distance_matrix_" + str(userId) + ".png", dpi=300)
        plt.close()
        ...
        # xu: we need to scale the similarity
        # plot the similarity
        # K = minmax_scale(K)
        ...
        # plt.colorbar(im, fraction=0.04, pad=0.03)
        plt.tight_layout()
        plt.savefig(dataPath + "userData/distMatrix/similarity_matrix_" + str(userId) + ".png", dpi=300)
        plt.close()
        # distribution of similarity
        K_flattern = K.flatten()
        fig = plt.figure()
        sns.distplot(K_flattern)
        plt.savefig(dataPath + "userData/distMatrix/similarity_distribution_" + str(userId) + ".png", dpi=300)
        plt.close()
        ...
        fig = plt.figure()
        plt.matshow(L)
        plt.savefig(dataPath + "userData/distMatrix/similarity_matrixL_" + str(userId) + ".png", dpi=300)
        plt.close()
        # distribution of similarity
        L_flattern = L.flatten()
        fig = plt.figure()
        sns.distplot(L_flattern)
        plt.savefig(dataPath + "userData/distMatrix/similarity_distributionL_" + str(userId) + ".png", dpi=300)
        plt.close()

        # Eigendecomposition
        Eval, Evec = np.linalg.eig(L)

        # xu: convert Eval and Evec to real
        Eval = Eval.real
        Evec = Evec.real

        print("# of eigenvalues : ", len(set(Eval.tolist())))
        ...
        if g == -1:
            ...
            fig = plt.figure()
            plt.plot(range(gMax - gMin + 1)[::2], rhog.tolist()[::2], marker='o', color='r')
            ...
            plt.tight_layout()
            plt.savefig(dataPath + "userData/distMatrix/kmeans_score_" + str(userId) + ".png", dpi=300)
            plt.close()
